...
class ToCheckpoint(Tool):
    ...
    def cleanup(self):
        self._logger.info("Processed {0} alerts.  New blocks: {1}  New revoke: {2}".format(self._totalProcessed,self._totalBlocked,self._totalRevoked))
    ...
    def _block(self,alert):
        ...
        if(self.is_valid_ipv4(addr) or self.is_valid_ipv6(addr)):
            if (addr not in self._rules and addr not in self._blockIPs):
                ...
                self._totalBlocked=self._totalBlocked+1
                self._modified=True
                return True
        return False
    ...
    def _getDuration(self, alert):
        ...
        if(duration==None or int(duration)==0):
            return self.getConfig().getDefaultDuration()
        return duration
    ...
    def _revoke(self,alert):
        ...
        if(self.is_valid_ipv4(addr) or self.is_valid_ipv6(addr)):
            if(addr in self._rules):
                ...
                self._totalRevoked=self._totalRevoked+1
                self._modified=True
                return True
        return False
