from . import rc
#try:
#    from ssh import SSHClient,WarningPolicy
#except ImportError:
...
class CheckpointConfig(ToolConfig):
    ...
    def updateRules(self,blockIPs,unblockUIDs):
        ...
        f=open('cp.txt','w')
        for ip in blockIPs:
            f.write("add -t "+str(ip._timeout)+" -a r -l r -o "+self._originator+" -c "+eventID+" ip -d "+ip._addr+"\n")
            f.write("add -t "+str(ip._timeout)+" -a d -l r -o "+self._originator+" -c "+eventID+" ip -s "+ip._addr+"\n")
