...
class PaloAltoConfig(ToolConfig):
    ...
    def __init__(self, config,csvToolInfo,unhandledCSV):
        ...
        if('default_duration' in config):
            ...
            self._logger.warning("default_duration not specified in the configuration, setting default_duration to 86400 seconds")
        ...
        if(self._apiKey == None):
            ...
            if(self._userName != None or self._password != None):
                self._logger.error("Only one of the api_key or username/password must be specified in the configuration")
                hasError=True
        if(hasError):
            self.disable()
    ...
    def _ensureDBExists(self):
        ...
        if(not os.path.exists(dbfile)):
            ...
            c=conn.cursor()
            c.execute("create table blocks (ip text primary key, detect_time integer(11), start_time integer(11), end_time integer(11), duration integer(11))")
