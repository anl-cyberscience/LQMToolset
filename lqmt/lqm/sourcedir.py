...
class DirectorySource(Source):
    ...
    def __init__(self, config):
        ...
        if('post_process' not in config):
            ...
            if post_process not in ["move", "delete", "track", "nothing"]:
                raise ConfigurationError("Invalid value for key: 'post_process' in section: 'Source.Directory': "+post_process)
    ...
    def logStatistics(self, numAlerts):
        self._logger.info("dirs: {0} NumDirs: {1} NumFiles: {2}".format(",".join(self._dirs),self._ftp._numDirs,self._ftp._numFiles))
