...
class ProcessHandlerTrackFile(ProcessedHandler):
    """This ProcessedHandler class implements adding the file's name to a list of files processed that is stored in the directory.
    This is not a terribly efficient meth as the file fill grow and so will the directory size"""
    ...
    def enteringDirectory(self, path):
        pfile=os.path.join(path,".processed.txt")
        try:
            self._processed={}
